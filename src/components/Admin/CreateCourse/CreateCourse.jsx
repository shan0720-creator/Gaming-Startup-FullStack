...
import {Grid,Box, Heading, VStack,Container,Input, Select,Image,Button} from "@chakra-ui/react"
...
const CreateCourse = () => {
     ...
     const categories = ['Unreal4  Developement ', 'Unreal5  Developement', 'Unity Master Class', 'Virtus RPG Course', 'FreeCodeCamp', 'Unreal Sensei']
    ...
    <Grid css={{
       ...
       templateColumns={['1fr','5fr 1fr']}>
        ...
        <form>
            ...
            <Select focusBorderColor='purple.300' value={category} onChange={e=>setCategory(e.target.value)} mt="30px">
            <option value="">Category</option>
            {categories.map(item=>(
                <option key ={item} value={item}>{item}</option>
            ))}
            </Select>
            <Input accept="image/*"  type={"file"} placeholder="Type Your Password" focusBorderColor="purple.300" css={{'&::file-selector-button':{...fileUploadCss,color:"purple",}}} onChange={changeImageHandler} mt={"30px"}/>
            {imagePrev && (
